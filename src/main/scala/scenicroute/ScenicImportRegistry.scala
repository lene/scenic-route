...
/** A GraphHopper [[ImportRegistry]] that adds two custom encoded values — `cqi_quality` and
  * `scenic_quality` — backed by precomputed [[WayQuality]] scores.
  *
  * All other EV names are forwarded to a [[DefaultImportRegistry]] delegate so standard GraphHopper
  * attributes continue to work.
  *
  * @param scores
...
final class ScenicImportRegistry(scores: Map[Long, WayQuality]) extends ImportRegistry:
  ...
  /** Returns an [[ImportUnit]] for the named encoded value.
    ...
    * [[DecimalEncodedValueImpl]] slots wired to a [[ScenicTagParser]]. Any other name is delegated
    * to [[DefaultImportRegistry]].
    */
